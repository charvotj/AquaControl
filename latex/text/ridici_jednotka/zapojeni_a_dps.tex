...
\label{sec:ridici-jednotka-schema-a-dps}
    ...
    \subsection{Napájecí obvod}
        ...
        Výstupní napětí je dáno poměrem odporového děliče R9 a R10. Na pinu FB je udržováno konstantní napětí \(U_{FB} = \qty{0.8}{V}\), z~této podmínky pak vyplývá vztah pro výstpní napětí:
        \begin{equation}
    ...
    \subsection{Deska plošných spojů}
        ...
        \subsubsection{Měnič napětí}
            Při tvorbě části desky s~měničem napětí typu buck je potřeba dodržet obecná doporučení týkající se návrhu \acs{dps} se spínanými zdroji. Tato doporučení jsou spolu s~příklady detailně popsána v~katalogovém listu součástky~\cite{lm5148-datasheet} a dalších návrhových příručkách firmy Texas Instruments (výrobce čipu LM5148). Pohled na výsledný návrh propojení součástek na \acs{dps} je zobrazen na obr.~\ref{fig:lm5148buck-edit-png}. Zobrazena je zde pouze strana součástek (vrchní strana DPS). 
